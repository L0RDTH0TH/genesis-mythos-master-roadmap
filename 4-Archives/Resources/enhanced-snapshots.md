...
links: [[2026-02-25-enhanced-snapshots]]
---

# Enhanced Snapshots — Backup Integration

Short reference for **including in-vault snapshots in external backups** and where to get paths. Full architecture: [[2026-02-25-enhanced-snapshots]].
...
- **Where the paths are defined**: `~/.cursor/mcp.json` → `mcpServers` → `obsidian-para-zettel-autopilot` → `env`:
  - `SNAPSHOT_DIR` — e.g. `.../Backups/Per-Change`
  - `BATCH_SNAPSHOT_DIR` — e.g. `.../Backups/Batch`
