...
| **`sandbox-genesis-mythos-master`** (engine) | **Sandbox engine roadmap line** — aligns with **EAT-QUEUE lane `sandbox`**, Config **`lane_project_id: sandbox-genesis-mythos-master`**, `GMM_PROJECT_ROOT` for that project. | **Roadmap-only publish surface**: **`Roadmap/`** (including **`Roadmap/Execution/`** when execution track is active — see [Dual-Roadmap-Track](Dual-Roadmap-Track.md)) + sandbox anchor files from `1-Projects/sandbox-genesis-mythos-master/`. No global `.cursor/`, `scripts/`, or system `Docs/` on engine branches. |
| **`godot-genesis-mythos-master`** (engine) | **Godot engine roadmap line** — aligns with **EAT-QUEUE lane `godot`**, Config **`lane_project_id: godot-genesis-mythos-master`**. | Same roadmap-only policy as sandbox: **`Roadmap/`** (including **`Roadmap/Execution/`** when execution track is active) + anchors from `1-Projects/godot-genesis-mythos-master/`. No global `.cursor/`, `scripts/`, or system `Docs/` on engine branches. |

**Invariant:** Engine branches must **not** publish any canonical automation spine (`.cursor/`, `scripts/`, system `Docs/`). Collaborators who need the **true** agents/rules/skills use the **integration** branch only. Engine branches are **roadmap-first and roadmap-only** outputs.
...
## Rule-empty engine branches

**Integration tip** (**`iteration-2-roadmap-rules`**, or Config **`gitforge.integration_branch`**) is the only branch that carries the **full** automation spine: **`.cursor/`**, **`scripts/`**, and system **`Docs/`**.

When publishing an **engine** line (**`sandbox-genesis-mythos-master`** or **`godot-genesis-mythos-master`** on GitHub):

1. **`git switch`** your engine branch (or create it from the integration tip per lineage above).
2. Set **`GMM_PROJECT_ROOT`** to the vault engine project path.
3. Run **Step 1b — engine** only (**Roadmap + anchors**). Do not copy global automation surfaces.

This keeps GitHub **engine** branches strictly **roadmap+anchor-focused** while the **integration** branch remains the complete automation contract.
...
Use **Step 1 — integration** when checked out on **`gitforge.integration_branch`**. Use **Step 1b — engine** when checked out on **`sandbox-genesis-mythos-master`** or **`godot-genesis-mythos-master`** (roadmap-only publish).
...
**Prerequisites:** **`GMM_PROJECT_ROOT`** set to the engine path. Export checkout on the **engine** branch.
...
After **engine** sync: expect changes only under **`Roadmap/`** and the two anchor files (`<project-id>-goal.md`, `<project-id>-Roadmap-MOC.md`).
...
git add -A Roadmap
git add "${PROJ_ID}-goal.md" "${PROJ_ID}-Roadmap-MOC.md"
# Engine branches are roadmap-only; do not stage .cursor/scripts/system Docs here.
```
