...
| **URL whitelist (automatic §0)** | **Before** **`Task(research)`**, enforce [[.cursor/rules/agents/execution-research-whitelist|execution-research-whitelist]] — only **`https://en.cppreference.com/w/`** and **`https://cplusplus.com/reference/`**; hand-off scan **before** Research; **non-whitelisted URLs → `task_error` + honesty ledger**; **abort entire deepen**; no execution write. |
