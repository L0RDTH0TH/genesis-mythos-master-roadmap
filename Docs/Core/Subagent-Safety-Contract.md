...
- **Layer 1 (Queue/Dispatcher)**
  ...
  - SHOULD run **`python3 -m scripts.eat_queue_core.harness post_queue_gitforge`** **once** after **queue.mdc A.7** when **A.7a** gates pass, Second-Brain-Config **`gitforge.enabled`** is **true**, **`gitforge.harness_enabled`** is **true** (default), and **`effective_pipeline_mode`** is **`balance`** or **`quality`** (**`speed`** skips GitForge). On non-zero harness exit, follow **Proof-on-failure** (Errors.md; **`error_type: gitforge-harness-failure`**). **Legacy:** **`Task(subagent_type: "gitforge")`** only when **`gitforge.harness_enabled`** is **false**; on host rejection of `gitforge`, MAY use `generalPurpose` with the GitForge contract ([[.cursor/agents/gitforge|agents/gitforge.md]]). On legacy **`Task(gitforge)`** failure, follow **Proof-on-failure** (comms + Errors.md; **`error_type: gitforge-task-failure`**). GitForge failure MUST NOT roll back queue consumption.
...
**Invocation:** **Layer 1 only**, **once** per Part A run, **after** **[[.cursor/rules/agents/queue.mdc|queue.mdc]] A.7a** gates. Config: [[3-Resources/Second-Brain/Second-Brain-Config|Second-Brain-Config]] § **`gitforge`**. **Default:** deterministic **`python3 -m scripts.eat_queue_core.harness post_queue_gitforge`** (see [[3-Resources/Second-Brain/Docs/Queue-Harness-Architecture|Queue-Harness-Architecture]]). Contract reference: [[.cursor/agents/gitforge|agents/gitforge.md]]. **IRA**, **Validator**, **pipelines**, and **Layer 0** MUST NOT run the harness or call **`Task(gitforge)`**.

**Parallel dual-track (v1):** When **`parallel_execution.enabled`** is **true**, the harness **must** acquire **`{vault_root}/.technical/.gitforge.lock`** (timeout **`parallel_execution.gitforge.lock_timeout_seconds`**) before vault git/export — preferably via **`python3 scripts/gitforge_lock.py acquire`** from **`vault_root`** per **agents/gitforge.md**; if the lock is not acquired, **skip** and log per **agents/gitforge.md**. Hand-off may include **`parallel_track`**, **`parallel_branch_prefix`**, **`parallel_export_path`**.
