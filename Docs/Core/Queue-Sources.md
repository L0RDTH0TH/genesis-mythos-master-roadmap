...
- **Observability (multi-phase):** Watcher-Result lines for roadmap dispatches **should** include **`queue_pass_phase=initial|cleanup|inline`**, **`dispatch_ordinal=N`**, **`roadmap_pass_order=repair_first|forward_first`** in **`message`** or **`trace`** (still only **`status: success|failure`**). **`dispatch_ledger`** entries include **`queue_pass_phase`**. Audit **`line_removed`** may use **`disposition: consumed_inline_repair_drain`** for Pass 3. See [[3-Resources/Second-Brain/Logs|Logs]] § Run-Telemetry, [[3-Resources/Second-Brain/Docs/Queue-Audit-Log-Spec|Queue-Audit-Log-Spec]].
