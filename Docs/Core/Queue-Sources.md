...
**TL;DR** — **PQ** = prompt queue file resolved in [[.cursor/rules/agents/queue.mdc|queue.mdc]] **A.0x**: legacy **`.technical/prompt-queue.jsonl`** (also the **central pool** when **`queue.central_pool_fanout_enabled`** is true), or **`.technical/parallel/<track>/prompt-queue.jsonl`** when **`parallel_execution.enabled`** matches **`EAT-QUEUE lane sandbox|godot`**. Prompt Crafter and most append paths write the **pool**; **A.0.4** hydrates per-track **PQ** before dispatch. **A.7** removes consumed ids from **PQ** and, when fanout applies, from the **pool**. Pipeline modes append to **PQ** (usually the pool path). Task-Queue.md (`3-Resources/`) for task/roadmap modes. Validate before append; read-then-append only; on RESUME_ROADMAP append remove stale RESUME_ROADMAP lines **in the same PQ file** then append.
...
When **`parallel_execution.enabled`** is **true** and **`default_to_legacy`** is not **true**, Layer 1 uses **separate bundles** under **`.technical/parallel/<track>/`** for **`sandbox`** and **`godot`** lanes (same inner filenames: `prompt-queue.jsonl`, `eat_queue_run_plan.json`, `queue-continuation.jsonl`, `prompt-queue-audit.jsonl`, `tmp-prompt.json`, `eat-queue-decisions.jsonl`). **`EAT-QUEUE`** without **`lane`**, or lanes **`default`** / **`shared`** / **`core`** (when not a configured parallel track lane), keep **legacy** single **PQ** at **`.technical/prompt-queue.jsonl`**. When **`queue.central_pool_fanout_enabled`** is **true**, Layer 1 runs **`python3 -m scripts.eat_queue_core.pool_sync`** (**A.0.4**) so the **pool** (`.technical/prompt-queue.jsonl`) is filtered into the track **PQ** before **A.1**. Each **`parallel_execution.tracks[]`** row may set **`lane_project_id`** for **A.0z** / **A.2a.1** (roadmap state under **`1-Projects/<lane_project_id>/`** only). Layer 0 passes **`## parallel_context`** (including **`lane_project_root`** when configured) in the **`Task(queue)`** hand-off when the user says **`EAT-QUEUE lane <lane>`** and Config matches (see [[.cursor/rules/always/dispatcher.mdc|dispatcher.mdc]]). **GitForge** serializes across tracks via **`.technical/.gitforge.lock`** (see [[3-Resources/Second-Brain/Docs/git-push-workflow-2026-04-02-0446|git-push-workflow]]). **Operator ritual:** Chat A → **`EAT-QUEUE lane sandbox`**; Chat B → **`EAT-QUEUE lane godot`**; author **`queue_lane`** on JSONL lines to match the intended track. Full operator contract: [[3-Resources/Second-Brain/Docs/Dual-track-EAT-QUEUE-Operator|Dual-track-EAT-QUEUE-Operator]].
