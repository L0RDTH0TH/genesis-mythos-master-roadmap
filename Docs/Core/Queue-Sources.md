...
**TL;DR** — **PQ** = prompt queue file resolved in [[.cursor/rules/agents/queue.mdc|queue.mdc]] **A.0x**: legacy **`.technical/prompt-queue.jsonl`** (also the **central pool** when **`queue.central_pool_fanout_enabled`** is true), or **`.technical/parallel/<track>/prompt-queue.jsonl`** when **`parallel_execution.enabled`** matches **`EAT-QUEUE lane sandbox|godot`**. Prompt Crafter and most append paths write the **pool**; **A.0.4** **merges** the lane-filtered pool into per-track **PQ** and **preserves lane-only** lines already on disk unless **`queue.pool_sync_strict_central_only`** is true. **A.7** removes consumed ids from **PQ** and, when fanout applies, from the **pool**. Pipeline modes append to **PQ** (usually the pool path). Task-Queue.md (`3-Resources/`) for task/roadmap modes. Validate before append; read-then-append only; on RESUME_ROADMAP append remove stale RESUME_ROADMAP lines **in the same PQ file** then append.
...
| Mode type | Append to |
|-----------|-----------|
| INGEST_MODE, ROADMAP_MODE, RESUME_ROADMAP, DISTILL_MODE, EXPRESS_MODE, ORGANIZE_MODE, ARCHIVE_MODE, ATOMIZE_MODE, RESEARCH_AGENT, SEEDED_ENHANCE, … | **PQ** (legacy `.technical/prompt-queue.jsonl` or per-track path; see **A.0x**) |
...
## Parallel execution (dual-track PQ)

When **`parallel_execution.enabled`** is **true** and **`default_to_legacy`** is not **true**, Layer 1 uses **separate bundles** under **`.technical/parallel/<track>/`** for **`sandbox`** and **`godot`** lanes (same inner filenames: `prompt-queue.jsonl`, `eat_queue_run_plan.json`, `queue-continuation.jsonl`, `prompt-queue-audit.jsonl`, `tmp-prompt.json`, `eat-queue-decisions.jsonl`). **`EAT-QUEUE`** without **`lane`**, or lanes **`default`** / **`shared`** / **`core`** (when not a configured parallel track lane), keep **legacy** single **PQ** at **`.technical/prompt-queue.jsonl`**. When **`queue.central_pool_fanout_enabled`** is **true**, Layer 1 runs **`python3 -m scripts.eat_queue_core.pool_sync`** (**A.0.4**) so the **pool** (`.technical/prompt-queue.jsonl`) is **merged** into the track **PQ** before **A.1** (see **Lane-only preservation** below). Each **`parallel_execution.tracks[]`** row may set **`lane_project_id`** for **A.0z** / **A.2a.1** (roadmap state under **`1-Projects/<lane_project_id>/`** only). Layer 0 passes **`## parallel_context`** (including **`lane_project_root`** when configured) in the **`Task(queue)`** hand-off when the user says **`EAT-QUEUE lane <lane>`** and Config matches (see [[.cursor/rules/always/dispatcher.mdc|dispatcher.mdc]]). **GitForge** serializes across tracks via **`.technical/.gitforge.lock`** (see [[3-Resources/Second-Brain/Docs/git-push-workflow-2026-04-02-0446|git-push-workflow]]). **Operator ritual:** Chat A → **`EAT-QUEUE lane sandbox`**; Chat B → **`EAT-QUEUE lane godot`**; author **`queue_lane`** on JSONL lines to match the intended track. Full operator contract: [[3-Resources/Second-Brain/Docs/Dual-track-EAT-QUEUE-Operator|Dual-track-EAT-QUEUE-Operator]].

### Lane-only preservation (A.0.4 / `pool_sync`)

When **`queue.central_pool_fanout_enabled`** is true and **`queue.pool_sync_strict_central_only`** is **false** (default in [[3-Resources/Second-Brain/Second-Brain-Config|Second-Brain-Config]]), **`scripts.eat_queue_core.pool_sync`** **merges** the lane-filtered **central pool** into the per-track **PQ** and **retains** valid lines already in the track file whose **`id`** is absent from the pool and whose effective **`queue_lane`** matches that track (or **`shared`**). Central lines win on **`id`** collision. **`pool_sync_strict_central_only: true`** restores **overwrite-only** behavior (drops those lane-only rows). See [[.cursor/rules/agents/queue.mdc|queue.mdc]] **A.0.4**.
...
**Append-only:** **PQAUD** = **`dirname(PQ)` + `/prompt-queue-audit.jsonl`** when parallel bundle active; legacy override: Second-Brain-Config **`queue.audit_log_path`** when set (else **`.technical/prompt-queue-audit.jsonl`** in legacy single-file mode). When **`queue.audit_log_enabled`** is true, Layer 1 writes **`line_removed`** / **`line_appended`** per [[3-Resources/Second-Brain/Docs/Queue-Audit-Log-Spec|Queue-Audit-Log-Spec]]. Cursor: whitelist per-track audit paths under **`.technical/parallel/**`** if using globs, and **`!.technical/prompt-queue-audit.jsonl`** beside **`prompt-queue.jsonl`** for legacy.
...
- **Ordering:** lane hydration (**A.0.4**) runs before **A.1b**; hydration **merges** the central pool into the lane **PQ** (plus lane-only preservation when **`pool_sync_strict_central_only`** is false). Bootstrap remains the recovery path when there are no usable records.
...
- **Optional PQ shell (filesystem only):** [[.cursor/skills/queue-bootstrap-guard/SKILL.md|queue-bootstrap-guard]] may ensure the parent of **PQ** exists and create an **empty** queue file when the path is missing — **no** JSONL payload; **[[.cursor/rules/agents/queue.mdc|queue.mdc]]** **A.1b** remains the only place that may synthesize bootstrap **RESUME_ROADMAP** lines.
...
## prompt-queue.jsonl (**PQ**)

- **Location**: **PQ** — default **`.technical/prompt-queue.jsonl`**; per **[[.cursor/rules/agents/queue.mdc|queue.mdc]] A.0x** may be **`.technical/parallel/sandbox/prompt-queue.jsonl`**, **`.technical/parallel/godot/prompt-queue.jsonl`**, etc. (folder excluded from Obsidian via Settings → Files & Links → Excluded files; see Vault-Layout)
- **Curator backup mirror (non-authoritative):** Small Obsidian-visible duplicates live under [[3-Resources/Second-Brain/Backup-Mirrors-Curator/README|Backup-Mirrors-Curator]] so queue state remains **grepable** after restore; refresh when canonical JSONL changes.
...
- **Queue lanes (multi-track EAT-QUEUE):** Optional top-level **`queue_lane`** (string). Allowed values: Second-Brain-Config **`queue.allowed_lanes`** (default includes `default`, `shared`, `sandbox`, `godot`, `core`). **Authoring:** Prefer explicit **`"queue_lane": "default"`** for general-track lines so audits and logs show a lane without inferring from absence. If **`queue_lane`** is **omitted**, parsers treat effective lane as **`default`** for filtering. **`shared`** is for cross-track work (common contracts, interfaces, Phase 6 artifacts): **`EAT-QUEUE lane sandbox`** / **`lane godot`** process **`target_lane ∪ shared`**; **`EAT-QUEUE lane shared`** processes **only** `shared`; **`EAT-QUEUE lane default`** processes **only** `default` (no automatic union with `shared`). Plain **`EAT-QUEUE`** (no `lane` token) processes **all** lines. **Hand-off:** Layer 0 passes **`queue_lane_filter`** to Layer 1 when the user says **`EAT-QUEUE lane <name>`** (see [[3-Resources/Second-Brain/Docs/User-Flows/EAT-QUEUE-Flow|EAT-QUEUE-Flow]], [[.cursor/rules/agents/queue.mdc|queue.mdc]] **A.2a**). **Validation:** Do **not** append lines whose **`queue_lane`** is set and **not** in **`queue.allowed_lanes`**. **A.7** merge uses a **pre-write re-read** of the JSONL file so parallel lane-scoped runs only drop their own **`processed_success_ids`** and do not overwrite other lanes’ lines.
...
- **Roadmap multi-dispatch (Layer 1 A.4c / A.5.0):** Second-Brain-Config **`queue.roadmap_pass_order`** controls how many **`RESUME_ROADMAP`** / **`RESUME_ROADMAP-*`** lines per **`project_id`** may run in **one** EAT-QUEUE invocation. **Familial presets:** [[3-Resources/Second-Brain/Docs/Core/Config-Profiles|Config-Profiles]] **`repair_strategy`** / **`speed_mode`** expand to **`queue.*`** / **`pipeline_mode`** via deepMerge (see **queue.mdc** **A.2** resolver bullet). **`repair_first`** (default): **one** roadmap dispatch per project per run (after repair-first sub-sort) — legacy behavior. **`forward_first`**: **initial pass** — up to **`queue.max_blocking_repair_preflight_per_project_per_run`** **blocking-repair** lines (repair-class with **`params.queue_blocking_repair: true`** or **`params.action`** `recal` / `handoff-audit`), then up to **`queue.max_forward_roadmap_dispatches_per_project_per_run`** **forward-class** lines (not repair-class); **cleanup pass** — up to **`queue.max_repair_roadmap_dispatches_per_project_per_run`** additional **repair-class** lines. Non-roadmap modes still dispatch in **initial pass** only (canonical order). **`queue.roadmap_refresh_between_roadmap_dispatches`** (default true): re-read roadmap state between roadmap **`Task`** calls. **Pass 3 (combined inline drain):** After passes 1–2, Layer 1 may run a **bounded** generation loop when **`inline_repair_pending`** (and **`queue.inline_a5b_repair_drain_enabled`** ≠ false) **or** when **`inline_forward_followup_pending`** (and **`queue.inline_forward_followup_drain_enabled: true`**). Re-read **`prompt-queue.jsonl`**, assign **`dispatch_pass: inline`** to **repair-class** lines (under shared repair budget with cleanup, or under a **separate Pass 3 repair budget** when **`queue.pass3_drain_appended_until_empty`** and **`queue.pass3_inline_repair_uses_separate_budget`** are **true**; **`queue.max_inline_a5b_repair_generations_per_run`** / dynamic **`inline_drain_max_gen`** caps waves), and optionally **`dispatch_pass: inline_forward`** to **forward-class** lines appended mid-run (**A.5c** / **A.5c.1**) when forward drain is on — capped by per-project forward dispatch limits (raised when **`pass3_drain_appended_until_empty`** via **`max_pass3_inline_forward_dispatches_per_project_per_run`**) and **`queue.max_inline_forward_followup_generations_per_run`**, with **`queue.inline_forward_drain_appended_ids_only`** (default true) restricting eligibility to ids first appended this run. See [[.cursor/rules/agents/queue.mdc|agents/queue.mdc]] **A.4c**, **A.5.0**.
- **Common misconception (Pass 3):** A **repair-class** line already on **PQ** from a **prior** run does **not** by itself set **`inline_repair_pending`**; Pass 3 is gated by **same-run** pending flags after **A.5b** / **A.5d** (etc.). See [[3-Resources/Second-Brain/Docs/User-Flows/EAT-QUEUE-Pass-3-Operator-Guide|EAT-QUEUE Pass 3 Operator Guide]].
- **Generate-eat parity (optional invariant):** For one EAT-QUEUE run, let **G** = the set of **`id`** values successfully appended to **`prompt-queue.jsonl`** from **A.5b** / **A.5c** / **A.5c.1** / **A.5c.2** / **A.5d** recovery (each append only when **A.5.0.1** mid-run cap allows). Either Layer 1 **dispatches** eligible forward/repair-class lines from **G** in the same run (within caps), **or** it **refuses** further appends when the mid-run cap is reached (**`queue_midrun_append_cap`**). When **`queue.pass3_drain_appended_until_empty`** is **true**, Pass 3 uses a larger **`inline_drain_max_gen`** from **|G|** + **`pass3_drain_generation_slack`**, separate Pass 3 repair tagging when configured, and logs **`inline_drain_incomplete`** if pending inline work remains after waves or **`max_roadmap_task_invocations_per_eat_queue_run`** (**`eat_queue_roadmap_task_cap`**) blocks further **`Task(roadmap)`**. Roadmap returns: when drain mode is on, prefer **one** **`queue_followups.next_entry`** per successful deepen where possible ([[.cursor/agents/roadmap|agents/roadmap]] return contract).
- **Overnight / anti-circle:** **`queue.max_roadmap_task_invocations_per_eat_queue_run`** (default **25**) — Layer 1 **A.5.0.2** blocks **`Task(roadmap)`** after that many invocations **in one** EAT-QUEUE; undispatched lines remain. Watcher **`skipped: roadmap_task_cap`**; audit **`entry_skipped_no_dispatch`** with **`reason: roadmap_invocation_cap_exceeded`**. **`roadmap.conceptual_max_deepen_per_subphase`**, **`roadmap.stagnation_window_runs`**, **`roadmap.stagnation_confidence_delta_max_percent`** — computed in **roadmap-deepen**, slice exit in RoadmapSubagent **(3d)**; see [[3-Resources/Second-Brain/Parameters|Parameters]] § Anti-Circling & Overnight Safety. **Empty-queue bootstrap dedup:** **A.1b** step **8a** — skip append when continuation **`workflow_cursor_at_completion`** matches current **`workflow_state`** (**`last_auto_iteration`** + **`current_subphase_index`**); **A.5e** merges **`workflow_cursor_at_completion`** after successful **RESUME_ROADMAP deepen** ([[3-Resources/Second-Brain/Docs/Queue-Continuation-Spec|Queue-Continuation-Spec]]).
...
- **Observability (multi-phase):** Watcher-Result lines for roadmap dispatches **should** include **`queue_pass_phase=initial|cleanup|inline|inline_forward`**, **`dispatch_ordinal=N`**, **`roadmap_pass_order=repair_first|forward_first`** in **`message`** or **`trace`** (still only **`status: success|failure`**). **Layer 1 post–little-val:** when **`Task(validator)`** runs after a pipeline dispatch, append **two** lines with the **same** **`requestId`**: **`segment: VALIDATE`** first, then the **primary** disposition line **without** **`segment: VALIDATE`** (Obsidian Watcher may only surface the last line per id — both remain in the file). **`dispatch_ledger`** entries include **`queue_pass_phase`**. Audit **`line_removed`**: **`consumed_inline_repair_drain`** (Pass 3 repair), **`consumed_inline_forward_followup_drain`** (Pass 3 forward). See [[3-Resources/Second-Brain/Logs|Logs]] § Run-Telemetry, [[3-Resources/Second-Brain/Docs/Queue-Audit-Log-Spec|Queue-Audit-Log-Spec]].
...
**Execution track — parallel spine (vault paths):** When **`effective_track: execution`**, structural roadmap notes must live under **`Roadmap/Execution/<mirrored folders>/`** (same relative path as conceptual under **`Roadmap/`**). Flat `Phase-*.md` files directly on **`Roadmap/Execution/`** when conceptual uses nested **`Phase-*/`** folders are a **policy violation**. Copy-paste **`user_guidance`** / **`prompt`** snippet: [[3-Resources/Second-Brain/Docs/Dual-Roadmap-Track|Dual-Roadmap-Track]] § **Execution path hand-off**. Normative: **roadmap-deepen** § Execution track path rule.

**Control plane v2 (return merge):** Roadmap **Task** returns may include a fenced **`control_plane_observability`** YAML block per [[3-Resources/Second-Brain/Docs/Control-Plane-Heuristics-v2|Control-Plane-Heuristics-v2]] §2. Layer 1 **A.5e** merges **`gate_waived`**, **`waiver_reason`**, **`effective_cap_used`**, **`routing_decision`**, **`stagnation_severity`**, **`stagnation_cluster_id`**, and related keys into the active **QCONT** path (**A.0x**); **A.5h** appends a row to **`dirname(PQ)/control-plane-nightly.jsonl`** when **`parallel_track`** is set, else Config **`queue.control_plane_v2.nightly_ledger_path`** (default **`.technical/control-plane-nightly.jsonl`**) when **`roadmap.control_plane_v2.nightly_ledger.enabled`** is **true**.
...
- **Erratum (2026-03-30):** Legacy **`queue.conceptual_skip_auto_repair_primary_codes`** merged execution-only and pseudo-code into one list. **Split:** **`queue.conceptual_execution_only_advisory_codes`** (execution-only fence — e.g. **`missing_roll_up_gates`**, **`safety_unknown_gap`**) drives **A.5b.0** skip of **A.5b.1–3** on **needs-work-only** conceptual outcomes. **`queue.conceptual_pseudo_code_advisory_codes`** lists the pseudo-code family for **A.5b.0d** tagging; those primaries are **not** in the execution-only fence (repair path when **hard block** + tiering allow).

- **Watcher-Result (`segment: VALIDATE`):** If post–little-val is **needs-work-only** (not a hard block per `queue.mdc` A.5b) and resolved **`primary_code`** is listed in **`queue.conceptual_execution_only_advisory_codes`** ([[3-Resources/Second-Brain-Config|Second-Brain-Config]] § `queue`), **`message` must** begin with: **`execution-deferred (advisory); out of scope for conceptual completion —`** (then severity, codes, report path). If **`primary_code`** is only in **`queue.conceptual_pseudo_code_advisory_codes`**, **do not** use that prefix as the execution-deferred banner — follow **A.5b.0d** and optional **`conceptual_track_compliant`**. If a **hard block** applies **and** **`reason_codes`** also list execution-deferred codes, state the hard-block outcome first, then note that execution-deferred codes are advisory on conceptual (**`queue.mdc` A.5b.0a**).
...
- **Config knobs:** **`queue.conceptual_execution_only_advisory_codes`**; **`queue.conceptual_pseudo_code_advisory_codes`**; **`queue.conceptual_force_build_on_repeated_gap`** (default **false** — no auto-escalation into rollup/CI artifact chases on conceptual); **`queue.medium_gap_escalation_threshold`** (default **4** when escalation is re-enabled). See **`queue.mdc` A.5b.0**, **A.5b.0d**, **A.5b.5**, **A.6**.
...
Authoritative behavior: [[3-Resources/Second-Brain/Docs/Validator-Tiered-Blocks-Spec|Validator-Tiered-Blocks-Spec]]; processor steps: `.cursor/rules/agents/queue.mdc` **A.4** (repair-first sub-sort), **A.5b** (post–little-val append + **`inline_repair_pending`**), **A.5.0** Pass 3 (inline drain), **A.7** (rewrite).
...
**A.7 tiered policy (processed_success_ids):** When post–little-val yields a **hard block** and **A.5b** successfully **appends** a **repair** line for the same intent, the **original** queue entry id is **consumed** (included in `processed_success_ids`); the new repair line has a **new** `id`. When **`queue.inline_a5b_repair_drain_enabled`** is not **false**, that new line is normally **`Task(roadmap)`**-dispatched in the **same** EAT-QUEUE run (**Pass 3**) before **A.7** removes consumed lines; if Pass 3 is off, capped out, or skipped, the repair line remains for a **future** run. Do **not** set `queue_failed: true` on the original entry for that reason alone. If hard block occurs and **no** repair line can be appended (missing `project_id`, non-roadmap `validation_type`, I/O failure), the processor SHOULD set **`queue_failed: true`** on the original line (or log **Errors.md** + **#review-needed**) so the user sees a stuck entry.
