...
| **`Roadmap/Execution/`** | **Execution roadmap** subtree: parallel folder tree to the conceptual phase layout under `Roadmap/` (same [Roadmap Structure](Roadmap%20Structure.md) naming under `…/Roadmap/Execution/`). All deepen/recal structural writes for the execution track occur here. **Not** hidden via `.cursorignore` — readability preserved; immutability of conceptual notes is enforced by Cursor rules and [[3-Resources/Second-Brain/Docs/Dual-Roadmap-Track|Dual-Roadmap-Track]]. |
