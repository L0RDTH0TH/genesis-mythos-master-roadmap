...
### GitForge (`gitforge` Config block)

- **Spec:** [[3-Resources/Second-Brain/Docs/git-audit-log|git-audit-log]]; agent [[.cursor/agents/gitforge|gitforge.md]]; rule [[.cursor/rules/agents/gitforge|gitforge.mdc]].
- **Config:** [[3-Resources/Second-Brain-Config|Second-Brain-Config]] → **`gitforge`** (`enabled`, `export_repo_root`, `integration_branch`, `invoke_on_empty_queue`, `invoke_only_on_clean_success`, `modes`).
- **Behavior:** Layer 1 invokes **`Task(subagent_type: "gitforge")`** **once** after prompt-queue **A.7** when enabled and gates pass **and** **`effective_pipeline_mode`** is **`balance`** or **`quality`**. **`speed`** skips GitForge. Hand-off **`mode`** is always **`balance`**; **`source_pipeline_mode`** records **`balance`** vs **`quality`**. Config **`gitforge.modes.balance`** drives tag/export flags.
...
**Dynamic vs static RESUME-ROADMAP params:** Dynamic per-iteration params (derived inside `effectiveParams` on each deepen run, never written back into queue entries as sticky overrides) include **token_cap**, **research_max_tokens**, **research_synth_cap_tokens**, **research_result_limit**, **max_depth**, **branch_factor**, **batch_subphases**, **granularity**, and the effective `enable_research` / action choice when `action: "auto"`. When **`deepen_multi_artifact` is not `true`**, shape knobs must not expand beyond **one** structural roadmap note per deepen — see roadmap-deepen skill and Queue-Sources **deepen_multi_artifact**. Static or sticky params (per project or crafted run) include **queue_next** (sticky-true except when suppressed by handoff gate or explicit false via crafter/wrapper), **profile** name, hard caps such as **context_window_tokens**, the effective **enable_context_tracking** flag, and the **highlight_angles set** (weights may be dynamic but the set itself is not auto-extended mid-run). Any param that influences safety gates (snapshots, destructive vs non-destructive, context tracking on/off, hard window limits) must not be made dynamic in a way that bypasses the existing rules in this file and in `auto-roadmap` / `roadmap-deepen`.
...
- **Config keys (with defaults, overrideable via queue params for thresholds/heuristics):**
  ...
  - **deepen_multi_artifact** (bool, default **false** when absent) — when false, roadmap-deepen mints **one** structural phase-tree note per queue pass; set **true** (or use profile **deepen-aggressive**) to allow **max_depth** / **branch_factor** / **batch_subphases** batching.
  - **max_depth**, **branch_factor** — only multi-mint when **`deepen_multi_artifact === true`**; otherwise effective branch width is 1; see roadmap-deepen and Queue-Sources.
...
- **`roadmap_track`** (on `roadmap-state.md`): `conceptual` (default when absent) \| `execution`. When `execution`, RESUME-ROADMAP deepen uses `Roadmap/Execution/workflow_state-execution.md` and `Roadmap/Execution/roadmap-state-execution.md` (both at the **Execution root** only). New execution phase notes must live under **`Roadmap/Execution/…`** with a **parallel spine**: **mirror** the conceptual folder hierarchy under `Roadmap/` (**do not flatten** phase `.md` files at the `Roadmap/Execution/` root). See **roadmap-deepen** § Execution track path rule, [[Roadmap Structure|Roadmap Structure.md]] § Execution track – parallel spine, [[3-Resources/Second-Brain/Docs/Dual-Roadmap-Track|Dual-Roadmap-Track]] § Execution path hand-off.
...
- **`harness_validation_mode`** (`advisory` \| `strict`, default **`advisory`**): Layer 1 **A.5i** after each pipeline **Task** disposition, before primary **Watcher-Result** (**queue.mdc**). Outcomes **`pass`**, **`advisory`**, **`strict_fail`** — see [[3-Resources/Second-Brain/Docs/Harness-Patterns-and-Guidelines|Harness-Patterns-and-Guidelines]] §4. **`strict`** upgrades missing **`nested_subagent_ledger`**, attestation violations, and missing **`blocked_scope`** on hard-block paths to **`nested_attestation_failure`** even when **`strict_nested_return_gates`** / **`strict_nested_ledger_all_pipelines`** are **false** (per **A.5i**).
- **`mutation_recovery_mode`** (`hard_stop` \| `restart_plan` \| `continue_best_effort`, default **`restart_plan`**): When **`harness verify`** reports **PQ** / pool byte drift vs a saved **`harness snapshot`**, Layer 1 follows **`recovery_hint`** in the verify JSON — refuse rewrite (**`hard_stop`**), re-run **`harness full_cycle`** from current disk once (**`restart_plan`**), or continue with latest snapshot + plan ids (**`continue_best_effort`** — can mask same-lane races). **Orthogonal** to **`harness_validation_mode`** (Task return semantics). See [[3-Resources/Second-Brain/Docs/Queue-Harness-Architecture|Queue-Harness-Architecture]].
