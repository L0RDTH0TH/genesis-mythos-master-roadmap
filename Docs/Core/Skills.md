...
| **vault-git-publish-checklist** | .cursor/skills/vault-git-publish-checklist/SKILL.md | operator / manual | — | Curator snapshot first, then export checkout; Step 1 vs 1b per [[3-Resources/Second-Brain/Docs/git-push-workflow-2026-04-02-0446|Git push workflow]] | Human publish order; links to [[.cursor/agents/gitforge|GitForge]] + workflow; does not replace subagent |
| **gitforge-operator** | .cursor/skills/gitforge-operator/SKILL.md | operator / manual | — | GitForge orientation/debug; branch + Config pointers; skip conditions | Links to [[.cursor/agents/gitforge|agents/gitforge.md]] only; manual [[3-Resources/Second-Brain/Docs/git-audit-log|git-audit-log]] rows when automation off |
...
| queue-churn-guard | .cursor/skills/queue-churn-guard.mdc | queue (Layer 1 / operator) | before append to PQ | Normalize mode vs handoff-audit; HANDOFF dedupe window; mirror fallback tags | Checklist per Parameters § Queue shield hardening; links queue.mdc A.2 / A.5b.0z / A.6 |
| **queue-bootstrap-guard** | .cursor/skills/queue-bootstrap-guard/SKILL.md | Layer 0 (optional pre-`Task(queue)`) | before **`Task(queue)`** when PQ path must exist on disk | Ensure **`dirname(PQ)`**; create **empty** file if missing — **no** JSONL; **A.1b** owns bootstrap lines | Filesystem ensure only; does **not** parse queue; forbidden: RESUME_ROADMAP synthesis; optional neutral Watcher-Result line |
| **telemetry-summary** | .cursor/skills/telemetry-summary/SKILL.md | queue (EAT-QUEUE Part A, post-A.7) | after **A.7**, before **A.7a** GitForge | Overwrite **`Docs/Core/Run-Telemetry-Summary.md`**; Watcher tail + eat-queue-decisions table; optional archive | Run **`scripts/generate_telemetry_summary.py`** when **`telemetry_summary`** gates pass; **`--dry-run`** for stdout only; see Second-Brain-Config § **telemetry_summary** |
