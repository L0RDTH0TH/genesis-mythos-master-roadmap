...
3. **Queue lines** — Append new work to the **central pool** **`.technical/prompt-queue.jsonl`** when fanout is enabled (Prompt Crafter already targets this path). Set **`queue_lane`** on JSONL entries per [[3-Resources/Second-Brain/Queue-Sources|Queue-Sources]] (Queue lanes). Use **`shared`** only when the work should run with **sandbox ∪ shared** or **godot ∪ shared** (see **A.2a**). Prefer explicit **`default`** on general work when not parallel. **A.0.4** **merges** the pool into the per-track **PQ** and **preserves** lane-only lines already in the track file (ids not in the pool, correct **`queue_lane`**) unless **`queue.pool_sync_strict_central_only`** is **true** — so hand-edits to the track **PQ** are no longer blindly wiped, but **mirroring new work to the central pool** remains the supported ritual to avoid drift. **Do not hand-edit** per-track **`prompt-queue.jsonl`** files for new work; append via the **central pool** and **`harness pool_sync`** (or other harness-driven paths) so **`eat_queue_core`** stays the single writer for normative queue bytes.
...
2. **Assuming zero git contention** — Vault has **one** `.git`. The **`post_queue_gitforge`** harness serializes via **`.technical/.gitforge.lock`**; the losing track **skips** git for that run (not a failure of the other track).
...
3. **GitForge harness** — After telemetry summary (when run), balance/quality runs invoke **`post_queue_gitforge`** once. If the lock is held, return is **`skipped`** with message like **`GitForge skipped — lock held by other track`** and an audit line — **queue consumption is not rolled back**.
4. **Deterministic lock helper** — The harness runs **`python3 scripts/gitforge_lock.py acquire`** before git and **`release`** in a **`finally`** (see [[.cursor/agents/gitforge.md|agents/gitforge.md]]). **`release`** clears the lock when the acquirer process has exited; if a **live foreign** process still holds the lock, exit code **1** means another track is active — do not delete manually unless you know that process is stuck.
