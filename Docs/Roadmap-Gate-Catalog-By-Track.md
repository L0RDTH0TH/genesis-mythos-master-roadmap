...
**Human framing (not new gates):** Layer 1 **Watcher-Result** lines for post–little-val **`roadmap_handoff_auto`** use the **`execution-deferred (advisory); out of scope for conceptual completion —`** prefix when **`primary_code`** is only in **`conceptual_skip_auto_repair_primary_codes`** ([[3-Resources/Second-Brain-Config|Second-Brain-Config]] § `queue`). Validator **`roadmap_handoff_auto`** reports include a **banner** after the title (see [[.cursor/rules/agents/validator.mdc|validator.mdc]]) so rollup/registry/CI rows are visibly **advisory** on conceptual. See [[3-Resources/Second-Brain/Queue-Sources|Queue-Sources]] § Conceptual track: Watcher-Result prefix and Config.
