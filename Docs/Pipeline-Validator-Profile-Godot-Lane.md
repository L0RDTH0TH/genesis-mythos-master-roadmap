---
title: Pipeline validator profile — Godot lane (execution overlay)
created: 2026-04-11
tags: [second-brain, validator, queue, roadmap, godot, execution]
para-type: Resource
status: active
links:
...
# Pipeline validator profile — Godot lane (execution overlay)
...
- **`project_id`** **`godot-genesis-mythos-master`** (Config **`parallel_execution.tracks[]`** **`lane_project_id`** for the godot track)
...
Operators may add a **`godot_lane_validator_overlay: true`** flag under **`parallel_execution.tracks`** for **`id: godot`** in [[3-Resources/Second-Brain-Config|Second-Brain-Config]] so Layer 1 can emit **`effective_profile_snapshot`** without hand-authored hints. **Default when absent:** if **`parallel_track`** and **`lane_project_id`** match this doc’s activation tuple, treat the overlay as **active** for resolver purposes.

---

## Cross-references

- [[.cursor/rules/agents/godot-execution-guard|godot-execution-guard]] — runtime enforcement for RoadmapSubagent.
