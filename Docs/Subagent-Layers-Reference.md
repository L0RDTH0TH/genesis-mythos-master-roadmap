...
| **Layer 1** | Queue / Dispatcher | For each queue entry: **must** call the matching Layer 2 pipeline via Task **when** the entry is a pipeline mode; **must** run ResearchSubagent **only when** the entry has mode RESEARCH-AGENT or RESEARCH-GAPS; **must** run ValidatorSubagent **only when** the entry has mode VALIDATE or ROADMAP_HANDOFF_VALIDATE; **may** run PromptCraftSubagent per Config + **A.5d** after a pipeline return with **`prompt_craft_request`**; **may** run **GitForge** once after prompt-queue **A.7** per **queue.mdc A.7a** when **`gitforge.enabled`** and **`effective_pipeline_mode`** is **`balance`** or **`quality`** (skipped for **`speed`**) (post-run git/export tail — not a queue-entry pipeline). | Never inlines pipeline or helper logic; never calls Layer 2 as nested agents. |
