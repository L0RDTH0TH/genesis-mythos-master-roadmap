...
| `explicit_skip_stall` | Layer 1 **A.5.0** stall-skip: line had **`queue_agent_may_skip_if_stall`** and policy fired; **no** `Task` ran — **`continuation_eligible: false`**; empty-queue bootstrap must **not** treat as unfinished-success signal. |
...
2. **`continuation_eligible: true` MUST NOT** appear when **`suppress_reason === target_reached`**, **`suppress_reason === conceptual_target_reached`**, **`suppress_reason === no_gain_pending_user_gates`**, or **`suppress_reason === explicit_skip_stall`**.
...
6. When **`suppress_reason === explicit_skip_stall`**, **`continuation_eligible` MUST be `false`** (stall-skip is not pipeline Success consumption).
7. When the Task return includes **`queue_followups.next_entry`**, **`suppress_followup` MUST be `false`** and **`suppress_reason` MUST NOT** imply “no follow-up” unless the follow-up object is intentionally void (invalid — do not do this).
8. **Success** + **`params.queue_next !== false`** on the queue entry **requires** **`queue_followups.next_entry`** on the Roadmap return **or** a **terminal** **`suppress_reason`** from the enum above — see Queue-Sources § **`effective_followup_required`**.
