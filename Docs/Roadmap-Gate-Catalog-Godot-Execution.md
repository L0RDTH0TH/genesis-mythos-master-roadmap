...
| **Verbatim grammar citation** | For each **new** construct, the execution note **must** include a short **authority** block: quoted **verbatim** passage from official docs **plus** stable URL whose prefix is **only** **`https://docs.godotengine.org/en/stable/`** (see [[.cursor/rules/agents/execution-research-whitelist|execution-research-whitelist]]). Paraphrase-only without quote **does not** satisfy this gate. |
