...
**V4 — Two entry paths:** (1) **Conversational-on-rails crafter** (trigger: "We are making a prompt" / "We are making a CODE prompt" / "We are making a ROADMAP prompt"): lock-first, profile-aware, resume gate and profile gate with options A–D, mid-session restart (resume | start fresh), final summary annotations for manual overrides. Use when users care about locks, profiles, and per-run nuance. (2) **Commander macro fast path** (e.g. "Craft Ingest Default", "Craft and Queue"): builds queue entries directly with pinned params; bypasses tmp-prompt, locks, and profile interplay. Use for one-click presets. Both paths validate against the same Queue-Sources and MCP contracts. See [[3-Resources/Second-Brain/Prompt-Crafter-Implementation-Notes-V4|Prompt-Crafter-Implementation-Notes-V4]] and [[3-Resources/Second-Brain/Second-Brain-User-Flows/Prompt-Crafter-Structure-Detailed|Prompt-Crafter-Structure-Detailed]].
...
```mermaid
flowchart TD
  Macro["Craft Prompt macro"]
  Macro --> Pipeline["Pipeline picker: ingest | organize"]
  Macro --> Profile["Profile picker: default | project-priority"]
  Pipeline --> Assembly["Assembly from prompt_defaults"]
  Profile --> Assembly
  Assembly --> Preview["Preview Assembly → temp note"]
  Assembly --> CraftQueue["Craft and Queue → validate"]
  CraftQueue -->|invalid| Log["Prompt-Log outcome invalid; no append"]
  CraftQueue -->|valid| Append["Append line to prompt-queue.jsonl"]
```
...
- **V4:** Commander macros **do not** consult or update the tmp-prompt scratchpad or RESUME-ROADMAP locks; they are a separate, fast path. For lock-first and profile interplay, use the conversational crafter (trigger phrases above).
...
```mermaid
flowchart TD
  Entry["Queue entry with params"]
  Entry --> Merge["Merge: queue → user_guidance → Config → MCP"]
  Merge --> Contract["Contract: rationale_style enum, max_candidates, context_mode"]
  Contract -->|invalid| Err["Skip; Errors.md; Watcher-Result failure"]
  Contract -->|valid| Pass["Pass merged params to pipeline"]
  Pass --> Run["classify_para, propose_para_paths, subfolder_organize"]
```
...
```mermaid
flowchart LR
  Trigger["user_guidance or queue prompt or #guidance-aware"]
  Trigger --> Load["feedback-incorporate loads guidance"]
  Load --> Merge["Append to rationale_style if compatible"]
  Merge --> NoOverride["Do not override user_guidance"]
  NoOverride --> Log["Prompt-Log: params, merge_trace"]
```
...
flowchart TD
  ...
  MacroInvalid --> Abort["Abort; no queue append; Prompt-Log"]
  EATInvalid --> Skip["Skip entry; Errors.md; Watcher-Result"]
  SourceMissing --> Skip
  UnknownMode --> Skip
```
...
```mermaid
flowchart LR
  Craft["Craft and Queue"]
  EAT["EAT-QUEUE run"]
  Craft --> Log["Prompt-Log.md"]
  EAT --> Log
  Log --> Fields["timestamp, pipeline, params, source, outcome, merge_trace"]
  Fields --> Audit["Dataview: crafted runs this week"]
```
...
## Plan-mode Q&A user flow (detailed)

Flow reflects question order from [[3-Resources/Second-Brain/Prompt-Crafter-Param-Table|Prompt-Crafter-Param-Table]]. RESUME-ROADMAP optionals grouped into 4 blocks; CODE→INGEST shows all 6 params. Full question_order and param names: see Param-Table.

```mermaid
flowchart TD
  Start["User says We are making a prompt"]
  Start --> Kickoff["Q0: Which kind? A. CODE B. ROADMAP"]
  Kickoff --> CodePath["CODE"]
  Kickoff --> RoadmapPath["ROADMAP"]
  CodePath --> CodeMode["Choose pipeline: INGEST / ORGANIZE / DISTILL / EXPRESS / ARCHIVE / Task"]
  RoadmapPath --> RoadmapMode["ROADMAP MODE vs RESUME-ROADMAP"]
  CodeMode --> Q1["Q1: source_file"]
  Q1 --> Q2["Q2: context_mode"]
  Q2 --> Q3["Q3: max_candidates"]
  Q3 --> Q4["Q4: profile"]
  Q4 --> Q5["Q5: user_guidance / prompt"]
  Q5 --> Q6["Q6: rationale_style"]
  RoadmapMode --> Block1["Q1–4: action, project_id, phase, sectionOrTaskLocator"]
  Block1 --> Block2["Q5–11: context_tracking, research, handoff"]
  Block2 --> Block3["Q12–16: inject, token_cap, depth, branch_factor, profile"]
  Block3 --> Block4["Q17–18: userText, queue_next"]
  Q6 --> ManualText["Manual text phase for accepts_manual_text params"]
  Block4 --> ManualText
  ManualText --> Summary["Summary + plan + payload"]
  Summary --> AppendConfirm["Append to queue? Y/n"]
  AppendConfirm -->|Y| Validate["Validate payload"]
  AppendConfirm -->|n| NoWrite["No write; payload in plan"]
  Validate --> Route["Route: Task-Queue.md or prompt-queue.jsonl"]
  Route --> ReadAppendWrite["Read file, append one line, write back"]
  ReadAppendWrite --> Done["Done"]
```
