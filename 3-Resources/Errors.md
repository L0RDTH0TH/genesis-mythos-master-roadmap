...
### 2026-03-25 08:28 — queue dispatch resource_exhausted
...
| severity | medium |
| approval | n/a |
| timestamp | 2026-03-25T08:28:41.035682Z |
| error_type | task-dispatch-failed |
...
- Task(subagent_type: roadmap) failed with host error: `[resource_exhausted] Error`

#### Summary

- **Root cause:** Task tool returned `resource_exhausted` during dispatch.
- **Impact:** Entry was not processed and remains in `.technical/prompt-queue.jsonl`.
- **Suggested fixes:** Re-run EAT-QUEUE when capacity returns; if persistent, reduce concurrent workloads or restart the agent session.
- **Recovery:** None applied automatically (no pipeline ran; no vault changes were made for this entry).
