...
- **para-zettel-autopilot** — Trigger: Ingest/*.md open or batch, or INGEST MODE (with bootstrap). Pipeline: full-autonomous-ingest. Creates Decision Wrappers for mid/low confidence; wrappers get A–G from obsidian_propose_para_paths; user picks approved_option or approved_path; apply happens in Step 0 on EAT-QUEUE.
