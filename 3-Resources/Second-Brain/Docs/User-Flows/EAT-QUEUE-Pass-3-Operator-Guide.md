...
1. **Config gates:** `queue.inline_a5b_repair_drain_enabled` is not `false` (repair drain) **and/or** `queue.inline_forward_followup_drain_enabled === true` (forward follow-up drain). **Preferred:** familial shorthand such as **`speed_mode: balance + repair_strategy: repair_first`** (or omit familial keys — **Layer 1** applies that default bundle automatically) instead of setting **`queue.inline_*`** flags directly — see [[3-Resources/Second-Brain/Docs/Core/Config-Profiles|Config-Profiles]]. **Baseline:** the default bundle labels (**`balance` + `repair_first` + `forgiving`**) and matching flat defaults for **`roadmap_pass_order`**, **`inline_*`**, and **`validator.tiered_blocks_enabled`** are documented in [[3-Resources/Second-Brain/Second-Brain-Config|Second-Brain-Config]] § **`profiles`** and **`queue`** / **`validator`** YAML (Option B); override there for vault-wide behavior or use per-entry **`params`** overrides.
