---
title: EAT-QUEUE dry run — Layer 0–2 (RESUME_ROADMAP deepen, genesis-mythos-master)
created: 2026-04-02
tags:
  - second-brain
  - eat-queue
...
para-type: Resource
status: reference
source: "Chat-derived simulation; not a live queue run. See body for scope."
---

# EAT-QUEUE dry run — Layer 0 through Layer 2 (simulation only)
...
- [[.cursor/agents/roadmap]] requires **balance mode** deepen to attempt nested **`Task`** calls for **Validator** and **IRA** where applicable, with `nested_subagent_ledger` attestation. The dry run does **not** replace that contract — it only narrates flow.
...
- Layer 1 remains the **FINAL GATEKEEPER** on roadmap returns; exemplar docs (if added later) must **not** override [[.cursor/rules/agents/queue]] or [[.cursor/agents/queue]].

---

## Related

- Canonical railroad exemplar: [[3-Resources/Second-Brain/Docs/Examples/Roadmap-Deepen-Dry-Run-Reference|Roadmap-Deepen-Dry-Run-Reference]] (precedence block + alignment note).
