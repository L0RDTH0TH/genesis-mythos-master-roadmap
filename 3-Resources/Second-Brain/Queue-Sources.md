...
**TL;DR** — **PQ** = prompt queue file resolved in [[.cursor/rules/agents/queue.mdc|queue.mdc]] **A.0x**: legacy **`.technical/prompt-queue.jsonl`**, or **`.technical/parallel/<track>/prompt-queue.jsonl`** when Second-Brain-Config **`parallel_execution.enabled`** matches **`EAT-QUEUE lane sandbox|godot`**. Pipeline modes (INGEST_MODE, DISTILL_MODE, RESUME_ROADMAP, …) append to **PQ**. Task-Queue.md (`3-Resources/`) for task/roadmap modes (TASK_ROADMAP, TASK_COMPLETE, …). Validate before append; read-then-append only; on RESUME_ROADMAP append remove stale RESUME_ROADMAP lines **in the same PQ file** then append.
...
When **`parallel_execution.enabled`** is **true** and **`default_to_legacy`** is not **true**, Layer 1 uses **separate bundles** under **`.technical/parallel/<track>/`** for **`sandbox`** and **`godot`** lanes (same inner filenames: `prompt-queue.jsonl`, `eat_queue_run_plan.json`, `queue-continuation.jsonl`, `prompt-queue-audit.jsonl`, `tmp-prompt.json`, `eat-queue-decisions.jsonl`). **`EAT-QUEUE`** without **`lane`**, or lanes **`default`** / **`shared`** / **`core`** (when not a configured parallel track lane), keep **legacy** single **PQ** at **`.technical/prompt-queue.jsonl`**. Layer 0 passes **`## parallel_context`** in the **`Task(queue)`** hand-off when the user says **`EAT-QUEUE lane <lane>`** and Config matches (see [[.cursor/rules/always/dispatcher.mdc|dispatcher.mdc]]). **GitForge** serializes across tracks via **`.technical/.gitforge.lock`** (see [[3-Resources/Second-Brain/Docs/git-push-workflow-2026-04-02-0446|git-push-workflow]]). **Operator ritual:** Chat A → **`EAT-QUEUE lane sandbox`**; Chat B → **`EAT-QUEUE lane godot`**; author **`queue_lane`** on JSONL lines to match the intended track. Full operator contract: [[3-Resources/Second-Brain/Docs/Dual-track-EAT-QUEUE-Operator|Dual-track-EAT-QUEUE-Operator]].
