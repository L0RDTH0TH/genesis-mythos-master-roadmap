...
### Nested validator → IRA (`ira_after_first_pass`)

- **Who invokes IRA (important):** The **pipeline subagent** (Layer 2) calls IRA via a **nested `Task`** in the **same run** as its nested Validator calls. The **Queue does not** dispatch IRA—there is no IRA queue mode, and the Queue’s **post–little-val Validator** pass (after pipeline Success) **never** chains to IRA. See [[3-Resources/Second-Brain/Subagent-Safety-Contract|Subagent-Safety-Contract]] § Internal Repair Agent exception and `queue.mdc` A.6.
- **Config:** [[3-Resources/Second-Brain-Config|Second-Brain-Config]] → `nested_validator.ira_after_first_pass` (default **true**).
- **Queue override:** Optional **`params.ira_after_first_pass`** on any prompt-queue entry whose pipeline runs the nested ValidatorSubagent + IRA cycle (INGEST_MODE, ARCHIVE_MODE, ORGANIZE_MODE, DISTILL_MODE, EXPRESS_MODE, ROADMAP_MODE, RESUME_ROADMAP, RESEARCH_AGENT, etc.). This is only a **parameter** passed into the pipeline hand-off; it does **not** mean the Queue runs IRA.
- **Effective value:** Treat as **false** only when **`params.ira_after_first_pass === false`** or **Config** sets `nested_validator.ira_after_first_pass: false`. Otherwise **true** (including when the key is omitted).
- **When true (default):** After the **first** nested validator pass in that run, the pipeline **always** runs **IRA once** (repair plan may be empty), applies gated `suggested_fixes`, re-runs **little val** (CODE pipelines only), then calls ValidatorSubagent a **second** time with `compare_to_report_path`. **Research** skips little val between IRA and the second validator.
- **When false (legacy):** Skip IRA and the second validator **only if** the first pass is a clean **`log_only`** with no actionable gaps; any other first-pass verdict still runs the full cycle. See [[3-Resources/Second-Brain/Subagent-Safety-Contract|Subagent-Safety-Contract]] § Internal Repair Agent exception.
...
- **Roadmap pass order / repair drain:** `queue.roadmap_pass_order` (`repair_first` \| `forward_first`), caps **`max_forward_roadmap_dispatches_per_project_per_run`**, **`max_repair_roadmap_dispatches_per_project_per_run`**, **`max_blocking_repair_preflight_per_project_per_run`**, **`roadmap_refresh_between_roadmap_dispatches`**, **`stall_skip_enabled`**, **`stall_skip_min_hard_block_returns_per_run`** — see [[3-Resources/Second-Brain-Config|Second-Brain-Config]] § **`queue`** and [[3-Resources/Second-Brain/Queue-Sources|Queue-Sources]] § Roadmap multi-dispatch. Observability: [[3-Resources/Second-Brain/Logs|Logs]] § Run-Telemetry multi-phase note.
...
**Dual roadmap track (conceptual vs execution):** See [[3-Resources/Second-Brain/Vault-Layout|Vault-Layout]] § Dual roadmap track, Conceptual-Amendments, **Conceptual-Decision-Records**, and [[3-Resources/Second-Brain/Docs/Dual-Roadmap-Track|Dual-Roadmap-Track]] (Definitions). **Watcher / queue (conceptual):** Post–little-val **`roadmap_handoff_auto`** messaging and Config **`queue.conceptual_skip_auto_repair_primary_codes`**, **`conceptual_force_build_on_repeated_gap`**, **`medium_gap_escalation_threshold`** — see [[3-Resources/Second-Brain/Queue-Sources|Queue-Sources]] § Conceptual track: Watcher-Result prefix and Config.
...
- **`strict_nested_return_gates`** (default **false**): When **true**, the Queue **must not** add an entry to **`processed_success_ids`** if the pipeline return indicates success with **`little_val_ok: true`** but (1) **`validator_context`** is missing or invalid for a gated mode (INGEST_MODE, ARCHIVE MODE, ORGANIZE MODE, DISTILL_MODE / BATCH_DISTILL, EXPRESS_MODE / BATCH_EXPRESS, ROADMAP_MODE, RESUME_ROADMAP, RESEARCH_AGENT), or (2) parseable **`nested_subagent_ledger`** violates **Attestation invariants** (e.g. **`nested_validator_first`**, **`nested_validator_second`**, **`ira_post_first_validator`**, or **`research_pre_deepen`** with **`outcome: invoked_ok`** or **`invoked_empty_ok`** and **`task_tool_invoked: false`** — **`error_type: ledger_semantic_attestation_failure`**). When **false** (legacy), `queue.mdc` may skip the post–little-val validator and still consume; Layer 1 **should** emit **soft** logs when **`validator_context`** is absent (**`validator_context_missing_soft`**) or when a hollow ledger pattern is detected (**`ledger_semantic_attestation_soft`**). **Migration:** run with **false** until returns reliably include **`validator_context`** and honest ledgers, then set **true**.
