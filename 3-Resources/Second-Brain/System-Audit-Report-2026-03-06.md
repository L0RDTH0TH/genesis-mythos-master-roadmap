...
| obsidian_propose_para_paths | **Built** | MCP-Tools documents params (path, context_mode, rationale_style, max_candidates); descriptor is `propose_para_paths.json` (no `obsidian_` prefix in filename). | Naming is server-specific; doc reference is correct. |
