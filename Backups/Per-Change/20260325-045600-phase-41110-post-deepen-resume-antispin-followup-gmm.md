...
| **`H_canonical` / repo harness** | **WitnessRefHash_v0** bytes + registry row + repo emission | Repo artifact + hash spec | **TBD** — explicit in **TL;DR** scope |

**Next run hint (operator):** When **Ctx Util % ≥ 80%** after a deepen, prefer **`RESUME_ROADMAP` `handoff-audit`** under the **D-060** matrix over another heavy **`deepen`** — see [[workflow_state]] **`## Log`** row **2026-03-25 04:55** for this queue entry.
