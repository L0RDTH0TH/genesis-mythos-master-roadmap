...
- [ ] Run **GMM-DLG-01** before claiming any regen interleaving story in new tertiaries.
  - **Evidence to record:** scan date + “no `Operator pick logged` under **D-044**” (or paste sub-bullet if pick exists).
- [ ] Run **GMM-TREE-01** before minting any `phase-4-1-*` tertiary roadmap file.
  - **Evidence to record:** glob / folder listing result + cite **D-059** row state (still pending both forks unless sub-bullet says otherwise).
