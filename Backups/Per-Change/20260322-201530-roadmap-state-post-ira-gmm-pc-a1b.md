...
> **Nested validation:** First / compare-final **`roadmap_handoff_auto`** report paths recorded in Run-Telemetry / parent **`nested_subagent_ledger`** for this dispatch (host **`Task(validator)`** + **`Task(internal-repair-agent)`** when available).
