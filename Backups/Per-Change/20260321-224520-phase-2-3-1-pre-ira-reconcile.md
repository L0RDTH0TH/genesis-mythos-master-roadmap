...
function EMG2_LoreSimAlignmentScore(ledger_slice):
  // Reads frozen paths only; formula versioned in frontmatter when F is set
  lore := EXTRACT_JSON_PATH(ledger_slice, LORE_FLAGS_PATH_EXAMPLE)
  sim  := EXTRACT_JSON_PATH(ledger_slice, SIM_COUNTERS_PATH_EXAMPLE)
  return AlignmentFn_v0(lore, sim)   // returns 0..100; floor F is TBD
