...
handoff_readiness_scope: "Tertiary **3.2.1** + **3.2.2** draft `DmOverrideIntent_v0` / `RegenRequest_v0` + P1–P6 preconditions + **regen-before-merge** ordering; golden vectors + registry still TBD (**D-041** / **D-042**)"
...
**Acceptance sketch:** Override events are logged in deterministic order; regen paths re-run golden-safe subsets or emit fail-closed denials per policy table (TBD).
