...
- **Repair-first + `inline_a5b` (Repair-Heavy hygiene path):** When **`repair_strategy: repair_first`** and merged **`queue.inline_a5b_repair_drain_enabled`** is **true** (from **Second-Brain-Config** / explicit **`queue`** on the entry), **`expanded_flat`** must carry those **`queue.*`** values into the effective map **before** **Layer 1** evaluates **A.5.0** Pass 3 gates — so **Config** does **not** spuriously disable repair drain. **Stale repair hygiene (documented softening):** When both conditions hold, **Layer 1** **may** treat **pre-existing** repair-class **`RESUME_ROADMAP`** lines on **PQ** (e.g. **repair-l1-hygiene** / **handoff-audit** hygiene) as eligible to set **`inline_repair_pending = true`** at **start of Pass 3 re-tag**, so Pass 3 inline drain can run **on the next EAT-QUEUE pass** **without** a fresh **A.5b** append **this** run — **soft** same-run requirement for hygiene cases. **When used:** emit telemetry **`inline_repair_pending_from_stale`**. **Normative baseline** remains **[[.cursor/rules/agents/queue.mdc|queue.mdc]]** **A.5.0**; this bullet is the **Repair-Heavy** vault contract for **queue processor** behavior **without** editing **A.5.0** prose in the rule file.
...
2. **Second-Brain-Config** flat sections (`queue`, `validator`, **`queue_continuation`**, `snapshot`, `pipeline_mode`, `validator_profiles`, …).
