...
1. **Load guidance:** From note `user_guidance` first; if missing and queue entry has `prompt`, use `prompt` as guidance for this run. When the queue entry also has an **`agent_reasoning`** field (AI-only snippets from crafter C choices), you may append it as a clearly marked secondary block in the internal guidance context (e.g. "AI reasoning from prior C choices: …") but must not merge it into `user_guidance` or treat it as human input.
...
6. **Logging:** In pipeline log (Ingest-Log, Organize-Log, Distill-Log): `guidance_used: true | guidance_length: N`; when truncated add `guidance_truncated: true`; when ignored for safety add `guidance_ignored: safety`. When `agent_reasoning` was present and included in context, add a lightweight flag such as `agent_reasoning_used: true`. Optional: first 80 chars of guidance for traceability.
...
- [[.cursor/rules/always/confidence-loops|confidence-loops]] — approved: true, async re-run
- [[.cursor/rules/context/auto-eat-queue|auto-eat-queue]] — queue entry prompt, pre-dispatch
- [[3-Resources/Cursor-Skill-Pipelines-Reference|Cursor-Skill-Pipelines-Reference]] — where guidance is passed in pipelines
- [[3-Resources/Second-Brain/Parameters|Parameters]] — user_guidance, guidance_conf_boost
