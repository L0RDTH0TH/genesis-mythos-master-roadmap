...
**EAT-QUEUE / queue-based runs**: When the run was triggered by **EAT-QUEUE**, **Process queue**, **eat cache** / **EAT-CACHE** (or by pasted EAT-CACHE payload), the trigger is the queue (`.technical/prompt-queue.jsonl`) or the pasted payload, not Watcher-Signal. The agent must append **at least one** line per consumed queue **`requestId`** (primary disposition) to `3-Resources/Watcher-Result.md` in the same format (requestId, status, message, trace, completed). **Layer 1 (Queue subagent):** when post–little-val **`Task(validator)`** runs for a pipeline entry, append **two** lines sharing the **same** **`requestId`**: **`segment: VALIDATE`** first, then the primary disposition **without** **`segment: VALIDATE`** (see [[.cursor/rules/agents/queue.mdc|queue.mdc]] **A.5b** / **A.6**). The Obsidian Watcher plugin may only show the **last** line for a given **`requestId`**; both lines are still valid for audit. Each queue entry has an `id` field that serves as the requestId. See `.cursor/rules/context/auto-eat-queue.mdc` for the full queue processor flow. **Queue and task-queue runs are laptop-originated**; mobile does not send queue entries (mobile = observe + fill Ingest only; see Mobile-Migration-Spec).

## On run finish

**On run finish** (success or failure), append **one or more lines** to `3-Resources/Watcher-Result.md` (Layer 1 EAT-QUEUE may append **two** lines per **`requestId`** when post–little-val ran — see above):
