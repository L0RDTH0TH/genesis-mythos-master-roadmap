...
- **You must ALWAYS**:
  ...
  - On **every** final return, emit fenced YAML **`nested_subagent_ledger:`** per [Nested-Subagent-Ledger-Spec](3-Resources/Second-Brain/Docs/Nested-Subagent-Ledger-Spec.md) (including **Attestation invariants**) and [.cursor/agents/ingest.md](.cursor/agents/ingest.md) § **nested_subagent_ledger (required)**; follow the **pre-return checklist** and **Ledger attestation** there (no Success with **`little_val_ok: true`** without **`validator_context`** + ledger when applicable; no false-green **`invoked_ok`** + **`task_tool_invoked: false`** on mandated nested helper steps; **attempt-before-skip** — required nested **`Task`** or **`task_error`** + **`host_error_raw`**, never **`skipped`** + **`task_tool_invoked: false`** without allowlisted **`reason_code`** per agents/ingest.md).
