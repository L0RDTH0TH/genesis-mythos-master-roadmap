...
- **Export by branch type:** **`gitforge.integration_branch`** gets the **full system mirror** (`.cursor/` including **`.cursor/sync/`**, queue **`scripts/`** + **`gitforge_lock.py`**, full **`Docs/`** + **`Docs/Core/`** backbone + **`Docs/Second-Brain-User-Flows/`**). **Engine** branches (`sandbox-` / `godot-` lines) are **roadmap-only**: **Step 1b** (`Roadmap/` + anchors only), with no global `.cursor/`, `scripts/`, or system `Docs/` publish. See [[.cursor/agents/gitforge.md|agents/gitforge.md]] § **Export surfaces by branch type** and [[3-Resources/Second-Brain/Second-Brain-Config|Second-Brain-Config]] **`gitforge.export_contract`**.
