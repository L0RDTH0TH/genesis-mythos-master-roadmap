...
**TodoWrite:** Use **TodoWrite** to define run-scoped phase todos (e.g. read-params, branch-by-action, little-val-validator, run-telemetry). Set each phase to `in_progress` when starting and `completed` or `cancelled` when done. You **must not** return Success while any todo for this run is `pending` or `in_progress`.
...
4. **Branch by action:**
   - **deepen** — (1) Pre-deepen research when enabled: research must be **consumed**, not just linked.
     ...
     (3) Run **roadmap-deepen** (one step; update workflow_state). When **`params.queue_next !== false`**, the deepen skill returns a **`queue_followups`** payload — you **must** forward it verbatim in your **Task return** as **`queue_followups.next_entry`** (you **must not** drop it in prose-only summaries). Layer 1 **A.5c** appends it; you **must not** write `prompt-queue.jsonl` yourself.
