...
2. **Read queue**: Load `.technical/prompt-queue.jsonl` (or resolved per-track **PQ** from A.0x) or, when in EAT-CACHE mode, the queued_prompts payload. **Do not terminate prompt-queue runs on empty/missing/unreadable PQ**; treat as empty input and continue parse flow so **A.2 zero-valid-entry handling can invoke A.1b**.
...
6. **Watcher-Result and Run-Telemetry**: Baseline one line per disposition; when post–little-val **(b1)** ran, **two** lines for the same **`requestId`** (**VALIDATE** then primary). Tags in **message**/**trace** as in **A.6**. **A.1b visibility requirement:** if prompt-queue valid count is zero at A.2, run A.1b and emit an explicit bootstrap branch outcome (`empty-queue bootstrap appended`, `empty_queue_bootstrap_deduped_*`, `empty_queue_bootstrap_no_record`, or `OPERATOR ALARM: empty queue bootstrap failed`) before any "queue empty" conclusion.
7. **Rewrite queue**: **A.7** — remove **`processed_success_ids`**; stall-skipped lines **stay** on disk.
8. **GitForge (A.7a):** After **A.7** succeeds, if Second-Brain-Config **`gitforge.enabled`** is **true**, **A.7a** gates pass, and **`effective_pipeline_mode`** is **`balance`** or **`quality`** (not **`speed`**), run **`PYTHONPATH=<vault_root> python3 -m scripts.eat_queue_core.harness post_queue_gitforge --vault-root <vault_root> --handoff-file <handoff.json>`** (or **stdin** JSON) **exactly once** when **`gitforge.harness_enabled`** is **true** (default), with hand-off **`mode: balance`** and **`source_pipeline_mode`** set to the actual **`balance`** \| **`quality`** ([[.cursor/rules/agents/queue.mdc|queue.mdc]] **A.7a**). Include **`--parallel-context-file`** when **A.0x** supplied parallel context. **`speed`** → **skip** GitForge; then summarize and return. On harness **non-zero exit**: **Proof-on-failure** plus **`error_type: gitforge-harness-failure`**; never roll back **A.7**. When **`gitforge.harness_enabled`** is **false**, legacy **`Task(subagent_type: "gitforge")`** once + **`error_type: gitforge-task-failure`** on failure. Track **`any_prompt_queue_dispatch_failure`** during Part A; set **true** on any prompt-queue pipeline failure disposition. **Do not** call GitForge per pass or per entry.
