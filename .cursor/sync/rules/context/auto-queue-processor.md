---
description: "Process the task/roadmap queue (Task-Queue.md). Default handler is agents/queue.mdc via dispatcher; this file is kept for rollback — when routing points here, run the full flow below."
globs: []
alwaysApply: false
---
...
- **Default routing**: PROCESS TASK QUEUE (and EAT-QUEUE when intent is task queue) are routed by always/dispatcher.mdc to the **Queue/Dispatcher subagent** agents/queue.mdc. When that rule is active, run Part B (Task-queue flow) there.
- **When this file is the active handler** (e.g. after rollback): Run the full flow below (Read queue → Dispatch by mode → Log and update → Banner cleanup). Rollback = point system-funnels (and dispatcher) back to context/auto-queue-processor.mdc for PROCESS TASK QUEUE.
- **Queue**: Read **`3-Resources/Task-Queue.md`**. **Reference**: Cursor-Skill-Pipelines-Reference. **MCP safety**: mcp-obsidian-integration. Laptop-only queue.

## How to activate

When the instruction contains **PROCESS TASK QUEUE** (or **EAT-QUEUE** with task-queue intent) and **this rule** is the active handler, run the flow below. Read **Task-Queue.md**; if missing or no queue entries, treat as empty and exit.
