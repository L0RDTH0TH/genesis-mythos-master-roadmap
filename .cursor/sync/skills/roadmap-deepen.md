...
- **Single structural artifact (default, one queue / Pass 1 dispatch):** Unless **`params.deepen_multi_artifact === true`**, create or update **at most one** structural roadmap note in **step 5** (one new phase-tree `.md`, or one in-place refine per §3.1). After rollback, if the log says work completed but files are missing, mint **one** missing node and advance the cursor — do not remint an entire sibling set in one run. **Step 1b** template bootstrap and **step 6b** Conceptual-Decision-Records are not counted here.
...
- **params.deepen_multi_artifact** (optional, bool): When **absent** or **false** (default), **step 5** is **single-structural-mint only** — ignore multi-mint semantics below and in **`user_guidance`** (do not create several tertiaries/secondaries in one run; queue follow-ups carry additional siblings). When **true**, legacy batching below is allowed (for explicit aggressive profiles or operator opt-in).
- **params.max_depth** (optional, int): Max depth to create in one deepen run (e.g. 4 or 5). When absent, derive from **current_phase**: phase 1–2 → 2, phase 3–4 → 3, phase 5–6 → 4. When set **and** **`deepen_multi_artifact === true`**, loop step 5 until depth reaches max_depth or phase segment is complete; one Log row at end. When **`deepen_multi_artifact` is false**, treat as **single** step-5 iteration regardless of max_depth.
- **params.branch_factor** (optional, int): Target number of secondaries (or tertiaries per secondary) per node when **`deepen_multi_artifact === true`**. When **`deepen_multi_artifact` is false**, effective branch factor is **1** regardless of this param or Config defaults.
- **params.batch_subphases** (optional, array): e.g. `["3.2","3.3","3.4"]`. When **`deepen_multi_artifact === true`**, run step 5 serially over each; one workflow_state update at end (one Log row, Target = comma-separated list). Snapshot before batch. When **false**, use **only the first** element if the array is present, or ignore the param.
...
### Execution track path rule (mandatory)

When **`roadmap_track: execution`** (or **`roadmap-state-execution.md`** is the active roadmap state for the run):

1. Derive the **conceptual counterpart path** from **`roadmap-state.md`**, target frontmatter, and/or **`conceptual_counterpart`**.
2. **Execution target path** = **`Roadmap/Execution/`** (or `roadmapDir + execution_subfolder + "/"`) + the **exact same relative path** as the conceptual note under **`Roadmap/`** (preserve **every** folder level; use **`obsidian_ensure_structure`** for missing parents).
...
**Anti-pattern (explicit):** Skipping intermediate phase folders so a file lands at `Roadmap/Execution/Phase-3-2-UI-Integration-Roadmap-....md` (wrong — insufficient folder depth) instead of `Roadmap/Execution/Phase-3-Systems/Phase-3-2-UI-Integration-Roadmap-....md` when conceptual lives under `Roadmap/Phase-3-Systems/...`. **Correct:** `Roadmap/Execution/<same folders as conceptual>/<same basename>.md`.

See [[3-Resources/Second-Brain/Vault-Layout|Vault-Layout]] § Dual roadmap track and [[Roadmap Structure|Roadmap Structure.md]] § Execution track – parallel spine.
...
5. **Create folder(s) and note(s)**:
   - **Invariant (default):** If **`params.deepen_multi_artifact` is not `true`**, perform **at most one** structural create-or-refine in this step (one new roadmap note under the phase tree **or** one `obsidian_update_note` on the existing target in refine mode — §3.1). Do not loop siblings, do not recurse `max_depth`, do not honor “create N tertiaries” in **`user_guidance`** as a batch — record intent in Status/Next and rely on **`queue_followups`** for the next sibling.
   - When **`deepen_multi_artifact === true`** and **params.batch_subphases** is set (e.g. `["3.2","3.3","3.4"]`), run this step **serially** for each subphase in the batch; one **active** workflow_state update and one Log row at end (Target = comma-separated list). Snapshot before the batch.
   - When **`deepen_multi_artifact === true`** and **params.max_depth** or **params.branch_factor** is set: create up to **branch_factor** children at the current level, then recurse depth until **max_depth** (or phase-derived: 1–2→2, 3–4→3, 5–6→4). One snapshot before the multi-step run; one Log row at end.
   - Use **obsidian_ensure_structure** for the target phase folder under **phaseTreeRoot** (e.g. `…/Phase-N-<Name>/Phase-N-M-<SecondaryName>/` with prefix **phaseTreeRoot**). When **active_track** is **`execution`**, the path under **phaseTreeRoot** must match the **Execution track path rule**: same relative folders as the conceptual counterpart under **`Roadmap/`** (full mirror spine).
   - Create the roadmap note at the path per Roadmap Structure **under phaseTreeRoot** (e.g. `Phase-N-M-<SecondaryName>-Roadmap-YYYY-MM-DD-HHMM.md` with frontmatter `roadmap-level: secondary`, `phase-number: N`, `subphase-index: "N.M"`). When **active_track** is **`execution`**, add **`roadmap_track: execution`** and **`conceptual_counterpart`** pointing at the parallel conceptual path (derive by stripping the **`Execution/`** segment from the execution path to obtain the path under **`Roadmap/`** when minting a mirror).
...
7. **Iteration guidance, hard caps, and context-utilization gate**:
   ...
   - **Required: Request next step (when under caps and below context threshold):** When **params.queue_next !== false** (absent or undefined = true), the follow-up request is **required**, not optional. Only when **params.queue_next === false** (explicitly set) must the pipeline suppress follow-up.
     ...
     - Identity/sticky params to forward: **project_id** or **source_file**, **profile** (if present), effective **enable_context_tracking**, **queue_next**, **`deepen_multi_artifact`** when explicitly set on the entry (preserve operator batching opt-in), and other crafter-locked/static params. Do **not** sticky-write dynamic per-iteration knobs (token_cap, research_* budgets, branch_factor, max_depth, batch_subphases, etc.).
