...
- **Repair-first + `inline_a5b` (documentation; A.5.0 unchanged):** When **`repair_strategy: repair_first`** and merged **`queue.inline_a5b_repair_drain_enabled`** is **true** (from **Second-Brain-Config** / explicit **`queue`** on the entry), **`expanded_flat`** must carry those **`queue.*`** values into the effective map **before** **Layer 1** evaluates **A.5.0** Pass 3 gates — so **Config** does **not** spuriously disable repair drain. **queue.mdc** **A.5.0** still requires **`inline_repair_pending`** / **`inline_forward_followup_pending`** from **this run’s** **A.5b** / **A.5d** (and related) appends only; **do not** treat **`inline_a5b_repair_drain_enabled: true`** as authorizing Pass 3 without those flags, and **do not** infer pending from **pre-existing** repair-class lines that were **not** appended this run. For stale rows, operators **re-run** **EAT-QUEUE** after a qualifying append (see **[[3-Resources/Second-Brain/Docs/User-Flows/EAT-QUEUE-Pass-3-Operator-Guide|EAT-QUEUE Pass 3 Operator Guide]]**).
